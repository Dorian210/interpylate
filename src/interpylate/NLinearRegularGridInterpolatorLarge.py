...
class NLinearRegularGridInterpolatorLarge:
    ...
    def _make_mat(self):
        M = np.logical_not(
            np.logical_and(self.masks[None, :, :], self.masks[:, None, :]).any(axis=-1)
        ).astype("int")
    ...
    def hess(self, NDarray, continuous_inds, grad_too=False, evaluate_too=False):
        ...
        for axis1 in range(self.dim):
            ...
            for axis2 in range(axis1 + 1, self.dim):
                new2_masks, new2_coef_inds = (
                    self._change_masks_and_coef_inds_by_deriving(
                        new1_masks, new1_coef_inds, axis2
                    )
                )
